...
foreach diff of local diffs {
	...
	* Baseline
	#delimit ;
	local title "`label' `diff'-differenced, baseline";
	estmobility mobility_`suffix',
		xvar(act_cases10) begin("`begin'") end("`end'") constant(`constant') estnum(`=`k'+1')
		diff(`diff') title("`title'") exclude(weekend);
	#delimit cr
	file write record "(`=`k'+1') - `title'" _n

	* Adding state FE
	local title "`label' `diff'-differenced, adding state FE"
	if (`diff' == 0) {
		#delimit ;
		estmobility mobility_`suffix',
			xvar(act_cases10) begin("`begin'") end("`end'") statefe(1) estnum(`=`k'+2')
	...
	* Adding day FE
	local title "`label' `diff'-differenced, adding day FE"
	#delimit ;
	estmobility mobility_`suffix',
		xvar(act_cases10) begin("`begin'") end("`end'") statefe(`constant') dayfe(1) estnum(`=`k'+3')
		diff(`diff') title("`title'") exclude(weekend);
	#delimit cr
	file write record "(`=`k'+3') - `title'" _n

	* Adding weekends
	local title "`label' `diff'-differenced, adding weekends"
	#delimit ;
	estmobility mobility_`suffix',
		xvar(act_cases10) begin("`begin'") end("`end'") statefe(`constant') dayfe(1) estnum(`=`k'+4')
		diff(`diff') title("`title'");
	#delimit cr
	file write record "(`=`k'+4') - `title'" _n

	* Adding leads and lags
	local title "`label' `diff'-differenced, adding leads and lags"
	#delimit ;
	estmobility mobility_`suffix',
		xvar(act_cases10) begin("`begin'") end("`end'") statefe(`constant') dayfe(1) estnum(`=`k'+5')
...
// 	file write record "(`=`k'+7') - `title'" _n
	...
	* Baseline with rural share interaction
	local title "`label' `diff'-differenced, baseline with rural share interaction"
	#delimit ;
	estmobility mobility_`suffix',
		xvar(act_cases10) begin("`begin'") end("`end'") constant(`constant') estnum(`=`k'+6')
		diff(`diff') interact(rural) title("`title'") exclude(weekend);
	#delimit cr
	file write record "(`=`k'+6') - `title'" _n
	
	* Extending to 7-days after first day of SIP
	local title "`label' `diff'-differenced, baseline extd to 7d after SIP start"
	#delimit ;
	estmobility mobility_`suffix',
		xvar(act_cases10) begin("`begin'") end("`end'") daysafter(7) constant(`constant') estnum(`=`k'+7')
		diff(`diff') title("`title'") exclude(weekend);
	#delimit cr
	file write record "(`=`k'+7') - `title'" _n
	
	* 7 days after SIP with leads and lags
	local title "`label' `diff'-differenced, baseline extd to 7d after SIP start, w/leads and lags"
	#delimit ;
	estmobility mobility_`suffix',
		xvar(act_cases10) begin("`begin'") end("`end'") daysafter(7) constant(`constant') estnum(`=`k'+8')
